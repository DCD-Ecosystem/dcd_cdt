...
namespace dcd {
   ...
   dcd::public_key recover_key( const dcd::checksum256& digest, const dcd::signature& sig ) {
      ...
      dcd::public_key pubkey;
      if ( pubkey_size <= sizeof(optimistic_pubkey_data) ) {
         dcd::datastream<const char*> pubkey_ds( optimistic_pubkey_data, pubkey_size );
         pubkey_ds >> pubkey;
      } else {
         ...
         dcd::datastream<const char*> pubkey_ds( reinterpret_cast<const char*>(pubkey_data), pubkey_size );
         pubkey_ds >> pubkey;
   ...
   void assert_recover_key( const dcd::checksum256& digest, const dcd::signature& sig, const dcd::public_key& pubkey ) {
      ...
      auto sig_data = dcd::pack(sig);
      auto pubkey_data = dcd::pack(pubkey);
