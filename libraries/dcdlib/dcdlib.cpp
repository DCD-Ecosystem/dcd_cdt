...
#include "core/dcd/powers.hpp"
#include "contracts/dcd/system.hpp"
#include "contracts/dcd/privileged.hpp"

#include <algorithm>

extern "C" volatile uint64_t dcd_contract_name = 0;
extern "C" volatile void dcd_set_contract_name(uint64_t n) { dcd_contract_name = n; } // LLVM creates the call to this at the beginning of apply
...
namespace dcd {
   ...
   bool block_signing_authority_v0::is_valid()const {
      ...
      std::set<dcd::public_key> unique_keys;
   ...
   // privileged.hpp
   void set_blockchain_parameters(const dcd::blockchain_parameters& params) {
   ...
   void get_blockchain_parameters(dcd::blockchain_parameters& params) {
      char buf[sizeof(dcd::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      dcd::check( size <= sizeof(buf), "buffer is too small" );
      dcd::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }

   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = dcd::pack( prods );
