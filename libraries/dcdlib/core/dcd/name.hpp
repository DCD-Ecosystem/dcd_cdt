...
namespace dcd {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            dcd::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               dcd::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
...
inline constexpr dcd::name operator""_n() {
   constexpr auto x = dcd::name{std::string_view{dcd::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
