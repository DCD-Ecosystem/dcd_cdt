...
#include "../../core/dcd/varint.hpp"

#include "../../core/dcd/key_utils.hpp"

#include <algorithm>
#include <cctype>
#include <functional>

#warning "dcd::kv::table is designated as `alpha` and should not be used in production code"
...
template<typename T, dcd::name::raw TableName>
class table;
...
namespace internal {
   ...
   protected:
      ...
      index_base(dcd::name index_name, KF&& kf, T*) : index_name{index_name} {
   ...
   public:
     ...
     void value(void* val, void (*deserialize)(void*, const void*, std::size_t)) const {
      ...
      key_type key() const {
         ...
         return dcd::key_type{(char*)buffer, actual_value_size};
      }
...
private:
   ...
   public:
      ...
      reverse_iterator& operator++() {
         dcd::check(itr_stat != status::iterator_end, "incremented past the end");
         itr_stat = static_cast<status>(detail::itr_prev(itr));
         return *this;
      }
...
public:
   ...
   public:
      ...
      index(dcd::name name, KF&& kf) : index_base{name, kf, (T*)nullptr} {
      ...
      T operator[](const K& key) const {
         auto opt = get(key);
         dcd::check(opt.has_value(), "Key not found in `[]`");
         return *opt;
      }
    ...
    */
   void put(const T& value, dcd::name payer) {
      T old_value;
...
protected:
   ...
   void init(dcd::name contract, PrimaryIndex& prim_index, SecondaryIndices&... indices) {
      validate_types(prim_index);
      (validate_types(indices), ...);

      contract_name = contract;
      table_name = dcd::name{TableName};
