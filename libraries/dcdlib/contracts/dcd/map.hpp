...
namespace dcd::kv {
   ...
   namespace detail {
       ...
       * 
       * Note that this type in `dcd::kv::map` is defined as `dcd::kv::map::elem_t`.
       */
       ...
       * @ingroup keyvaluemap
       * 
       * @brief This struct represents the iterator for the `dcd::kv::map` data type.
       ...
       * 
       * Note that this iterator type in `dcd::kv::map` is defined as `dcd::kv::map::iterator_t`.
       * There is also a reverse iterator available, it is defined as `dcd::kv::map::reverse_iterator_t`.
   ...
   } // namespace dcd::kv::detail

   /**
    * @ingroup keyvaluemap
    *
    * @brief Defines an DCD Key Value Map
    ...
    */
   template <dcd::name::raw TableName, typename K, typename V, dcd::name::raw IndexName="map.index"_n>
   class [[dcd::table]] map {
      public:
         ...
         static const key_type& prefix() {
            static key_type prfx = dcd::detail::const_pack(magic, table_name, index_name);
            return prfx;
         }
