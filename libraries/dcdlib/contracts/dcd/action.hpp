...
#include "../../core/dcd/datastream.hpp"
#include "../../core/dcd/name.hpp"
#include "../../core/dcd/ignore.hpp"
#include "../../core/dcd/time.hpp"
...
namespace dcd {
   ...
   */
   ...
   struct permission_level {
      ...
      DCDLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      DCDLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         dcd::check( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
    ...
    * transfer_act{"dcd.token"_n, {st.issuer, "active"_n}}.send(st.issuer, to, quantity, memo);
    * // or
    * transfer_act trans_action{ "dcd.token"_n, {st.issuer, "active"_n}};
    * trans_action.send(st.issuer, to, quantity, memo);
    * @endcode
    */
   template <dcd::name::raw Name, auto Action>
   struct action_wrapper {
      template <typename Code>
      constexpr action_wrapper(Code&& code, std::vector<dcd::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const std::vector<dcd::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, dcd::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const dcd::permission_level& perm)
      ...
      static constexpr dcd::name action_name = dcd::name(Name);
      dcd::name code_name;
      std::vector<dcd::permission_level> permissions;
   ...
   template <dcd::name::raw Name, auto... Actions>
   struct variant_action_wrapper {
      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, std::vector<dcd::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const std::vector<dcd::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, dcd::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const dcd::permission_level& perm)
         : code_name(std::forward<Code>(code)), permissions({1, perm}) {}
...
::dcd::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::dcd::name(#NAME) )
 ...
 * DCD supports two communication models, inline and deferred. Inline actions are executed as part of the current transaction. This macro
 ...
 * @endcode
 *
 * The example above is taken from dcd.token.
 * This example:
 *       uses the passed in, dereferenced `this` pointer, to call this.get_self() i.e. the dcd.token contract;
 *       calls the dcd.token::transfer() action;
