...
namespace dcd {
   struct dsmalloc {
      ...
      char* operator()(size_t sz, uint8_t align_amt=16) {
         ...
         dcd::check(GROW_MEMORY(pages_to_alloc) != -1, "failed to allocate pages");
         return ret;
      }
...
void* malloc(size_t size) {
   void* ret = dcd::_dsmalloc(size);
   return ret;
}

void* memset(void*,int,size_t);
void* calloc(size_t count, size_t size) {
   if (void* ptr = dcd::_dsmalloc(count*size)) {
...
void* realloc(void* ptr, size_t size) {
   if (void* result = dcd::_dsmalloc(size)) {
