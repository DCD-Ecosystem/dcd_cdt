...
dcd::cdt::output_stream std_out;
dcd::cdt::output_stream std_err;
...
extern "C" {
   ...
   size_t _grow_memory(size_t size) {
      if ((___heap_ptr + (size*64*1024)) > (___heap_ptr + 100*1024*1024))
         dcd_assert(false, "__builtin_wasm_grow_memory");
   ...
   void _prints_l(const char* cstr, uint32_t len, uint8_t which) {
      for (int i=0; i < len; i++) {
         if (which == dcd::cdt::output_stream_kind::std_out)
            std_out.push(cstr[i]);
         else if (which == dcd::cdt::output_stream_kind::std_err)
            std_err.push(cstr[i]);
   ...
   int _wrap_main(int argc, char** argv) {
      ...
      intrinsics::set_intrinsic<intrinsics::prints_l>([](const char* cs, uint32_t l) {
            _prints_l(cs, l, dcd::cdt::output_stream_kind::std_out);
         });
      intrinsics::set_intrinsic<intrinsics::prints>([](const char* cs) {
            _prints(cs, dcd::cdt::output_stream_kind::std_out);
         });
      ...
      intrinsics::set_intrinsic<intrinsics::printn>([](uint64_t nm) {
            std::string s = dcd::name(nm).to_string();
      ...
      intrinsics::set_intrinsic<intrinsics::printhex>([](const void* data, uint32_t len) {
            ...
            if(buffer_size < len) dcd_assert( false, "length passed into printhex is too large" );
