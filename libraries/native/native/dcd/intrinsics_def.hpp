...
#include <dcd/types.h>
#include <dcd/security_group.h>

#include <type_traits>

namespace dcd { namespace native {
   ...
   auto create_function(std::index_sequence<Is...>) {
      return std::function<R(typename std::tuple_element<Is, Args>::type ...)>{
         [](typename std::tuple_element<Is, Args>::type ...) {
            dcd_assert(false, "unsupported intrinsic"); return (R)0;
         }
      };
   }
...
#define GENERATE_TYPE_MAPPING(name) \
   struct __ ## name ## _types { \
      using deduced_full_ts = decltype(dcd::native::get_args_full(::name)); \
      using deduced_ts      = decltype(dcd::native::get_args(::name)); \
...
#define GET_TYPE(name) \
   decltype(create_function<dcd::native::intrinsics::__ ## name ## _types::res_t, \
         dcd::native::intrinsics::__ ## name ## _types::deduced_full_ts>(dcd::native::intrinsics::__ ## name ## _types::is)),

#define REGISTER_INTRINSIC(name) \
   create_function<dcd::native::intrinsics::__ ## name ## _types::res_t, \
         dcd::native::intrinsics::__ ## name ## _types::deduced_full_ts>(dcd::native::intrinsics::__ ## name ## _types::is),
