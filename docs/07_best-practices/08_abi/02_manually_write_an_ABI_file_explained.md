...
- The Ricardian contracts should be housed in a file with the name `<contract name>.contracts.md` and the clauses should be in a file named `<contract name>.clauses.md`.
 ...
 - The option `-R` has been added to [`dcd-cpp`](../../03_command-reference/dcd-cpp.md) and [`dcd-abigen`](../../03_command-reference/dcd-abigen.md) to add "resource" paths to search from, so you can place these files in any directory structure you like and use `-R<path to file>` in the same vein as `-I` for include paths.
