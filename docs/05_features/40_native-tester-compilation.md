...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation. [`dcd-cc`](../03_command-reference/dcd-cc.md), [`dcd-cpp`](../03_command-reference/dcd-cpp.md) and [`dcd-ld`](../03_command-reference/dcd-ld.md) now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of dcd `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
...
class [[dcd::contract]] hello : public dcd::contract {
  public:
...
DCD_TEST_BEGIN(hello_test)
   ...
   intrinsics::set_intrinsic<intrinsics::read_action_data>(
         [](void* m, uint32_t len) {
            check(len <= sizeof(dcd::name), "failed from read_action_data");
            *((dcd::name*)m) = "hello"_n;
            return len; 
         });
   ...
   intrinsics::set_intrinsic<intrinsics::action_data_size>(
         []() {
            return (uint32_t)sizeof(dcd::name);
         });
...
- DCD_TEST_BEGIN(X) : This macro defines the beginning of a unit test and assigns `X` as the symbolic name of that test.
- DCD_TEST_END : This macro defines the end of a unit test.
- DCD_TEST(X) : This is used to run a particular named unit test `X` in the main function.
