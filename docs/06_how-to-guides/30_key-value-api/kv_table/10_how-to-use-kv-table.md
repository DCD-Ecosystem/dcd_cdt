...
To accomplish this task, define the user type which will be stored in the `kv table`, and extend the `dcd::kv::table` template class with a specialized definition based on the user defined type.
...
Complete the following steps to define the `address_table` type, based on the `dcd::kv::table`, which stores objects of type `person`:
...
2. Derive `address_table` from `dcd::`kv::table` class template. Pass the `person` user defined type as the type parameter for `dcd::`kv::table` base class and the name of the `kv table`, let’s say `kvaddrbook`.
3. Annotate `address_table` type with `[[dcd::table]]`, and make sure it is placed after the `struct` keyword but before the name of the type.
...
5. In the `address_table` constructor, call the `init(...)` base class method with the two parameters:
    1. The first parameter, of type `dcd::name`, is the contract that owns the table.
...
* You can [create one or more unique indexes](20_how-to-create-indexes-kv-table.md) using the `KV_NAMED_INDEX` macro or the `dcd::kv::table::index` template class.
* You can [create one or more non-unique indexes](20_how-to-create-indexes-kv-table.md) using the `KV_NAMED_INDEX` macro or the `dcd::kv::table::index` template class.
