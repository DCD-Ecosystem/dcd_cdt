...
## dcdlib
### Partitioning
In `dcd.cdt` v1.6.0, `dcdlib` will now be partitioned into 3 groups. These allow for finer grained allowance for particular modes of compilation.
  - CAPI
  - Contracts
  - Core
  - The `CAPI` will only be available to contract writers using `dcd-cc` and purely for `C` smart contracts.
  - The `Contracts` will be available to `dcd-cpp` for smart contract writing.
  - `Core` will be available to `dcd-cpp` for any of the modes (present and future).  
  - `dcd-cpp` in `fnative` mode will have access to all of these groups.

To access these new partitioned header files, use `<dcd/header_file.hpp>` instead of `<dcdlib/header_file.hpp>`.  Please note that all the old header files are still available at the old `dcdlib` directory, but these are deprecated and will be removed in v1.7.0.  Also, once you change one header file from `dcdlib` to `dcd` you will need to do so for all other occurrences at that point, because of some conflicts with the auto generated dispatcher.
...
This entire API is now only available to C developers (i.e. using `dcd-cc`), and all internal uses in `Core` and `Contracts` have been guarded behind the namespace `dcd::internal_use_do_not_use`.  

## dcdlib Contracts API
- `action.hpp`
...
- `print.hpp`
   ...
   - removed explicit `print` overloads for `dcd::symbol_code`, `dcd::fixed_bytes<T>` from here.
- `rope.hpp`
   - new file
...
There is no more need to add the `DCD_DISPATCH` macro to your smart contracts.  The compiler/linker will now automatically generate a dispatcher for you the proper `dcd::contract`, `dcd::action` and `dcd::on_notify` attributes are used.  Of course, if you don't have these attributes then you will still need to either use the old macro or hand write the `apply` function yourself.

### How the auto dispatcher will work
Given that you have marked your classes with the `dcd::contract` macro and any sub-contracts with the macro with the same given name (i.e. `dcd::contract("<contract name>")`) then any actions and notify handlers that are contained within these will be dispatchable by your smart contract.  This will allow for aggregate patterns for smart contract development and better separation of concerns.
...
If the dispatcher is in notification handling mode and if your contract receives an `dcd::onerror` notification, then the contract will assert with an error code. You can circumvent this check if you explicitly supply an error handler for it ([[dcd::on_notify("dcd::onerror")]]).
