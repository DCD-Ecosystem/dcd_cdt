...
namespace {
  // DcdApply - Mutate the apply function as needed
  struct DcdApplyPass : public FunctionPass {
    static char ID;
    DcdApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("dcd_wasm_entry") || F.getName().equals("apply")) {
         ...
         auto set_contract = F.getParent()->getOrInsertFunction("dcd_set_contract_name", AttributeList{}, Type::getVoidTy(F.getContext()), Type::getInt64Ty(F.getContext()));
...
char DcdApplyPass::ID = 0;
static RegisterPass<DcdApplyPass> X("apply_fixup", "Dcd Apply Fixups");

static void registerDcdApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new DcdApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerDcdApplyPass);
