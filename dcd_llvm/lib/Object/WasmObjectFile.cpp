...
Error WasmObjectFile::parseDcdABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   dcd_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("dcd abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".dcd_abi") {
     if (Error Err = parseDcdABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".dcd_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".dcd_notify") {
