...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_dcd_action = false;
  bool has_dcd_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("dcd_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("dcd_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("dcd_wasm_action"))
        has_dcd_action = true;
     if (F.hasFnAttribute("dcd_wasm_notify"))
        has_dcd_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection();
     std::string SectionName = ".dcd_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("dcd_wasm_abi")) {
           StringRef abi = F.getFnAttribute("dcd_wasm_abi").getValueAsString();
  ...
  if (has_dcd_action) {
     OutStreamer->PushSection();
     std::string SectionName = ".dcd_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("dcd_wasm_action")) {
           StringRef action_name = F.getFnAttribute("dcd_wasm_action").getValueAsString();
  ...
  if (has_dcd_notify) {
     OutStreamer->PushSection();
     std::string SectionName = ".dcd_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("dcd_wasm_notify")) {
           StringRef name = F.getFnAttribute("dcd_wasm_notify").getValueAsString();
