...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<DcdWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<DcdWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<DcdWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<DcdWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<DcdWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("dcd_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("dcd_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("dcd_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("dcd_wasm_action", FD->getDcdWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("dcd_wasm_notify", FD->getDcdWasmNotify().c_str());
     }
