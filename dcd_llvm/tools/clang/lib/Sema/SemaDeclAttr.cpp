...
static void handleDcdRicardianAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
   ...
   D->addAttr(::new (S.Context)
                 DcdRicardianAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdNotifyAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
   ...
   D->addAttr(::new (S.Context)
                 DcdNotifyAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdContractAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
    return;
  D->addAttr(::new (S.Context)
                 DcdContractAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdABIAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 DcdWasmABIAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdWasmActionAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 DcdWasmActionAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdWasmNotifyAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 DcdWasmNotifyAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdActionAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 DcdActionAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}

static void handleDcdReadOnlyAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  D->addAttr(::new (S.Context)
                 DcdReadOnlyAttr(AL.getRange(), S.Context, AL.getAttributeSpellingListIndex()));
}

static void handleDcdTableAttribute(Sema &S, Decl *D, const ParsedAttr &AL) {
  // Handle the cases where the attribute has a text message.
  StringRef Str;
  if (AL.isArgExpr(0) && AL.getArgAsExpr(0) &&
      !S.checkStringLiteralArgumentAttr(AL, 0, Str))
  ...
  D->addAttr(::new (S.Context)
                 DcdTableAttr(AL.getRange(), S.Context, Str,
                                AL.getAttributeSpellingListIndex()));
}
...
static void ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D,
  ...
  default:
    ...
    S.Diag(AL.getLoc(), diag::err_stmt_attribute_invalid_on_decl)
        << AL << D->getLocation();
    break;
  case ParsedAttr::AT_DcdWasmImport:
    handleSimpleAttribute<DcdWasmImportAttr>(S, D, AL);
    break;
  case ParsedAttr::AT_DcdWasmEntry:
    handleSimpleAttribute<DcdWasmEntryAttr>(S, D, AL);
    break;
  case ParsedAttr::AT_DcdIgnore:
    handleSimpleAttribute<DcdIgnoreAttr>(S, D, AL);
    break;
  case ParsedAttr::AT_DcdAction:
    handleDcdActionAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdReadOnly:
    handleDcdReadOnlyAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdTable:
    handleDcdTableAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdWasmABI:
    handleDcdABIAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdWasmAction:
    handleDcdWasmActionAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdWasmNotify:
    handleDcdWasmNotifyAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdContract:
    handleDcdContractAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdRicardian:
    handleDcdRicardianAttribute(S, D, AL);
    break;
  case ParsedAttr::AT_DcdNotify:
    handleDcdNotifyAttribute(S, D, AL);
    break;
