...
void Writer::createOutputSegments() {
  for (ObjFile *file : symtab->objectFiles) {
    if (!file->getDcdABI().empty())
       abis.push_back(file->getDcdABI());
    for (InputSegment *segment : file->segments) {
      if (!segment->live)
        continue;
...
static constexpr uint64_t DCD_COMPILER_ERROR_BASE = 8000000000000000000ull;
static constexpr uint64_t DCD_ERROR_NO_ACTION     = DCD_COMPILER_ERROR_BASE;
static constexpr uint64_t DCD_ERROR_ONERROR       = DCD_COMPILER_ERROR_BASE+1;
...
void Writer::createDispatchFunction() {
   ...
   auto create_if = [&](raw_string_ostream& os, std::string str, bool& need_else) {
      ...
      need_else = true;
      uint64_t nm = dcd::cdt::string_to_name(str.substr(0, str.find(":")).c_str());
   ...
   auto assert_sym = (FunctionSymbol*)symtab->find("dcd_assert_code");
   ...
   auto create_action_dispatch = [&](raw_string_ostream& OS) {
      ...
      for (ObjFile *File : symtab->objectFiles) {
        if (!File->getDcdActions().empty()) {
            for (auto act : File->getDcdActions()) {
      ...
      // do not fail if self == dcd
      writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
      writeUleb128(OS, 0, "self");
      writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
      encodeSLEB128((int64_t)dcd::cdt::string_to_name("dcd"), OS);
      writeU8(OS, OPCODE_I64_NE, "I64.NE");
      writeU8(OS, OPCODE_IF, "if receiver != dcd");
      writeU8(OS, 0x40, "none");
      ...
      if (assert_sym && assert_idx < symtab->getSymbols().size()) {
        ...
        writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
        encodeSLEB128((int64_t)DCD_ERROR_NO_ACTION, OS);
        writeU8(OS, OPCODE_CALL, "CALL");
   ...
   auto create_notify_dispatch = [&](raw_string_ostream& OS) {
      ...
      for (ObjFile *File : symtab->objectFiles) {
         if (!File->getDcdNotify().empty()) {
            for (auto notif : File->getDcdNotify()) {
      ...
      if (!has_onerror_handler) {
         // assert on onerror
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         uint64_t acnt = dcd::cdt::string_to_name("dcd");
         encodeSLEB128((int64_t)acnt, OS);
         writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
         writeUleb128(OS, 1, "code");
         writeU8(OS, OPCODE_I64_EQ, "I64.EQ");
         writeU8(OS, OPCODE_IF, "IF code==dcd");
         writeU8(OS, 0x40, "none");
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         uint64_t nm = dcd::cdt::string_to_name("onerror");
         encodeSLEB128((int64_t)nm, OS);
         writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
         writeUleb128(OS, 2, "action");
         writeU8(OS, OPCODE_I64_EQ, "I64.EQ");
         ...
         writeUleb128(OS, 0, "false");
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         encodeSLEB128((int64_t)DCD_ERROR_ONERROR, OS);
         writeU8(OS, OPCODE_CALL, "CALL");
         writeUleb128(OS, assert_idx, "code");
         writeU8(OS, OPCODE_END, "END");
         writeU8(OS, OPCODE_END, "END");
      ...
      if (not_cnt > 0) {
         ...
         for (auto const& notif0 : notify_handlers) {
            uint64_t nm = dcd::cdt::string_to_name(notif0.first.c_str());
            if (notif0.first == "*")
               continue;
            has_written = true;
   ...
   std::string BodyContent;
   {
      ...
      auto contract_sym = (FunctionSymbol*)symtab->find("dcd_set_contract_name");
      uint32_t contract_idx = contract_sym->getFunctionIndex();
      writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
      writeUleb128(OS, 0, "receiver");
      writeU8(OS, OPCODE_CALL, "CALL");
      writeUleb128(OS, contract_idx, "dcd_set_contract_name");
