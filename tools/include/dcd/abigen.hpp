...
namespace dcd { namespace cdt {
   class abigen : public generation_utils {
      ...
      void add_action( const clang::CXXRecordDecl* decl ) {
         abi_action ret;
         auto action_name = decl->getDcdActionAttr()->getName();
         ...
         if (translate_type(decl->getReturnType()) != "void") {
            ...
            */
            ...
            /*
               ...
               std::cout << "Error, currently in dcd.cdt v2.0 `auto` is not allowed for actions\n";
               throw abigen_ex;
            }
            */
         }
      ...
      void add_table( const clang::CXXRecordDecl* decl ) {
         // short circuit if we happen across `dcd::kv::map` declaration
         if (is_kv_map(decl))
            return;
         if (is_kv_table(decl)) {
            add_kv_table(decl);
         ...
         auto table_name = decl->getDcdTableAttr()->getName();
         if (!table_name.empty()) {
      ...
      void add_table( uint64_t name, const clang::CXXRecordDecl* decl ) {
         if (!(decl->isDcdTable() && abigen::is_dcd_contract(decl, get_contract_name())))
            return;
      ...
      std::string generate_json_comment() {
         std::stringstream ss;
         ss << "This file was generated with dcd-abigen.";
         ss << " DO NOT EDIT ";
         return ss.str();
      }
   ...
   class dcd_abigen_visitor : public RecursiveASTVisitor<dcd_abigen_visitor>, public generation_utils {
      private:
         bool has_added_clauses = false;
         abigen& ag = abigen::get();

      public:
         explicit dcd_abigen_visitor(CompilerInstance *CI) {
         ...
         virtual bool VisitCXXMethodDecl(clang::CXXMethodDecl* decl) {
            ...
            if (decl->isDcdAction() && ag.is_dcd_contract(decl, ag.get_contract_name())) {
               ag.add_struct(decl);
               ag.add_action(decl);
               for (auto param : decl->parameters()) {
         ...
         virtual bool VisitCXXRecordDecl(clang::CXXRecordDecl* decl) {
            ...
            if ((decl->isDcdAction() || decl->isDcdTable()) && ag.is_dcd_contract(decl, ag.get_contract_name())) {
               ag.add_struct(decl);
               if (decl->isDcdAction())
                  ag.add_action(decl);
               if (decl->isDcdTable())
         ...
         virtual bool VisitDecl(clang::Decl* decl) {
            if (const auto* d = dyn_cast<clang::ClassTemplateSpecializationDecl>(decl)) {
               if (d->getName() == "multi_index") {
                  ...
                  if (d->getSpecializedTemplate()->getTemplatedDecl()->isDcdTable())
                     ag.add_kv_map(d);
               }
            }
            return true;
   ...
   class dcd_abigen_consumer : public ASTConsumer {
      private:
         dcd_abigen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit dcd_abigen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new dcd_abigen_visitor(CI)), main_file(file), ci(CI) { }
