...
#include <dcd/utils.hpp>
#include <dcd/whereami/whereami.hpp>
#include <dcd/abi.hpp>
#include <dcd/ppcallbacks.hpp>
...
using namespace dcd;
using namespace dcd::cdt;

namespace dcd { namespace cdt {
   ...
   class dcd_codegen_visitor : public RecursiveASTVisitor<dcd_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit dcd_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class dcd::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_dcd_contract(decl, cg.contract_name)) {
               ss << "\n\n#include <dcd/datastream.hpp>\n";
               ss << "#include <dcd/name.hpp>\n";
               ss << "extern \"C\" {\n";
               ss << "__attribute__((dcd_wasm_import))\n";
               ...
               ss << "}\n";
               ss << "dcd::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               const auto& call_action = [&]() {
                  ss << decl->getParent()->getQualifiedNameAsString() << "{dcd::name{r},dcd::name{c},ds}." << decl->getNameAsString() << "(";
               ...
               call_action();
               if (return_ty != "void") {
                  ss << "const auto& packed_result = dcd::pack(result);\n";
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isDcdAction()) {
               ...
               validate_name(name, [&](auto s) {
                  CDT_ERROR("codegen_error", decl->getLocation(), std::string("action name (")+s+") is not a valid dcd name");
               });
               ...
               if (decl->isDcdReadOnly()) {
                  read_only_actions.insert(decl);
               }
            }
            else if (decl->isDcdNotify()) {
      ...
      class dcd_codegen_consumer : public ASTConsumer, public generation_utils {
      private:
         dcd_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit dcd_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new dcd_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  ss << "extern \"C\" {\n";
                  ss << "__attribute__((dcd_wasm_import))\n";
                  ss << "void dcd_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, dcd_wasm_entry, dcd_wasm_abi(";
                  ss << "\"" << quoted(cg.abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_dcd_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "dcd_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class dcd_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(std::make_unique<dcd_ppcallbacks>(CI.getSourceManager(), file.str()));
            return std::make_unique<dcd_codegen_consumer>(&CI, file);
         }
   };
