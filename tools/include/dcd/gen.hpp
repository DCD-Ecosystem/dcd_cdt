...
struct generation_utils {
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isDcdIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_dcd_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasDcdRicardian();
   }
   static inline bool has_dcd_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasDcdRicardian();
   }

   static inline std::string get_dcd_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getDcdRicardianAttr()->getName();
   }
   static inline std::string get_dcd_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getDcdRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getDcdActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getDcdNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_dcd_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isDcdContract())
         name = decl->getDcdContractAttr()->getName();
      else if (decl->getParent()->isDcdContract())
         name = decl->getParent()->getDcdContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_dcd_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isDcdContract()) {
         auto nm = decl->getDcdContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isDcdContract()) {
         auto nm = pd->getDcdContractAttr()->getName().str();
   ...
   inline bool is_kv_map(const clang::CXXRecordDecl* decl) {
      return decl->getQualifiedNameAsString().find("dcd::kv::map<") != std::string::npos;
   }
   ...
   inline bool is_kv_table(const clang::CXXRecordDecl* decl) {
      for (const auto& base : decl->bases()) {
         ...
         if (type.getAsString().find("dcd::kv::table<") != std::string::npos) {
            return true;
         }
      }
   ...
   inline bool is_kv_internal(const clang::CXXRecordDecl* decl) {
      ...
      const auto in_kv_namespace = fqn.find("dcd::kv") != std::string::npos;
