...
class Test(ABC):
    ...
    @abstractmethod
    def _run(self, dcd_cpp: str, args: List[str]):
        pass
    ...
    def run(self):
        ...
        dcd_cpp = os.path.join(self.test_suite.cdt_path, "dcd-cpp")
        self._run(dcd_cpp, args)
...
class BuildPassTest(Test):
    def _run(self, dcd_cpp, args):
        command = [dcd_cpp, self.cpp_file]
        command.extend(args)
...
class CompilePassTest(Test):
    def _run(self, dcd_cpp, args):
        command = [dcd_cpp, "-c", self.cpp_file]
        command.extend(args)
        res = subprocess.run(command, capture_output=True)
        self.handle_test_result(res)
...
class AbigenPassTest(Test):
    def _run(self, dcd_cpp, args):
        command = [dcd_cpp, self.cpp_file, "-abigen_output=''"]
        command.extend(args)
        res = subprocess.run(command, capture_output=True)
        self.handle_test_result(res)
