...
// example) cldcd push action todo getbyaccname '["todo"]' -p todo@active
[[dcd::action]]
...
// example) cldcd push action todo cntbyaccname '["todo"]' -p todo@active
[[dcd::action]]
...
// example) cldcd push action todo getbytask '["Start Blockchain"]' -p todo@active
[[dcd::action]]
...
// example) cldcd push action todo cntbytask '["Start Blockchain"]' -p todo@active
[[dcd::action]]
...
// example) cldcd push action todo getbychecked '["false"]' -p todo@active
[[dcd::action]]
...
// example) cldcd push action todo cntbychecked '["false"]' -p todo@active
[[dcd::action]]
...
/**
examples)
cldcd push action todo upsert '["ac8acfe7-cd4e-4d22-8400-218b697a4517", "todo", "Deploy Hello World Contract", false]' -p todo@active
cldcd push action todo upsert '["b7b0d09d-a82b-44d9-b067-3bae2d02917e", "todo", "Start Blockchain", false]' -p todo@active
cldcd push action todo upsert '["bf581bee-9f2c-447b-94ad-78e4984b6f51", "todo", "Write Hello World Contract", false]' -p todo@active
*/
[[dcd::action]]
todo_entry kv_todo::upsert(const std::string& uuid,
   ...
   if (itr != todo_entries.uuid.end()) {
      ...
      uint32_t created = dcd::current_time_point().sec_since_epoch();
...
void kv_todo::del(const std::string& uuid) {
   ...
   if (itr != todo_entries.uuid.end()) {
      ...
      dcd::print_f("todo_entry was successfully deleted from table.");
   } else {
      dcd::print_f("todo_entry not found in table.");
   }
}
